...
app.post('/items/:itemId', (req, res) => {
  const itemId = req.params.itemId;
  const { _method, updateName, updateDescription } = req.body;
  console.log(req.body);

  if (_method === 'PUT') {
      db.query('UPDATE items SET name = ?, description = ? WHERE id = ?', [updateName, updateDescription, itemId], (err) => {
          if (!err) {
              res.redirect('/');
          } else {
              res.status(500).send(err.message);
      ...
      db.query('UPDATE items SET is_active = 2 WHERE id = ?', [itemId], (err) => {
          if (!err) {
              res.redirect('/');
          } else {
              res.status(500).send(err.message);
